ServerIP=192.168.169.194
taskid=5
#!/bin/bash
#@Auth:wx
#@Time:2023/1/13
#@Refer:https://github.com/tangjie1/-Baseline-check
...
csvFile="checkList_${ServerIP}_${taskid}.csv"
rm $csvFile
echo "$csvFile"
...
tmp=`service sshd status | grep running | grep -v grep`
if [ -z "$tmp" ]; then
...
manual=$(($manual+1))
echo "6.35	检查是否关闭不必要的服务和端口	重要	自行判断	不必要的端口和服务会扩大系统的被攻击面，此检查项建议系统管理员根据系统情况自行判断	参考《Linux系统安全配置基线》对应章节	null	MANUAL		" >> "$csvFile"
print_manual_check
...
da=`date "+%Y-%m-%d %H:%M:%S"`
echo "Scantime:	$da" >> "$csvFile"
echo "ServerIP:	$ServerIP	" >> "$csvFile"
echo "taskid:	$taskid	" >> "$csvFile"
...
function get_basic_info {
    ...
    if [[ $(cat /proc/version) =~ "Red Hat" ]]; then
		...
		osVersion=`lsb_release -a | tr '\n' ' '`
	fi
    basic_info={\"netinfo\":\"$netinfo\",\"hostname\":\"$hostname\",\"macaddr\":\"$macaddr\",\"ipList\":\"$ipList\",\"kernelVersion\":\"$kernelVersion\",\"osVersion\":\"$osVersion\"}
    echo "netinfo:	$netinfo	" >> "$csvFile"
	echo "hostname:	$hostname	" >> "$csvFile"
	echo "macaddr:	$macaddr	" >> "$csvFile"
	echo "ipList:	$ipList	" >> "$csvFile"
	echo "kernelVersion:	$kernelVersion	" >> "$csvFile"
	echo "osVersion:	$osVersion	" >> "$csvFile"
	#echo $basic_info >> "$csvFile"
}
get_basic_info
...
iconv -f UTF-8 -t gb18030 $csvFile -o $csvFile #将文件内容编码转换后再存回原文件。
curl -X POST -F "file=@$csvFile" http://$ServerIP:8000/baselinecheck/hostupload/




# check_result={\"basic_info\":$basic_info,}
